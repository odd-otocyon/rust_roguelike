    ...
    for _ in 0..MAX_ROOMS {
        ...
        if !failed {
            ...
            if rooms.is_empty() {
                ...
                if rand::random() {
                    create_h_tunel(prev_x, new_x, prev_y, &mut map);
                    create_v_tunel(prev_y, new_y, new_x, &mut map);
                } else {
                    create_v_tunel(prev_y, new_y, prev_x, &mut map);
                    create_h_tunel(prev_x, new_x, new_y, &mut map);
                }
            }
            rooms.push(new_room);
        }
